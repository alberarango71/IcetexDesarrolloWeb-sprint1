...
function agregarProducto(nombre, precio) {
    ...
    if (productoExistente) {
        ...
        if (productosDisponibles[nombre] > 0) {
            productosDisponibles[nombre]--;
        }
    } else {
...
function actualizarCarrito() {
    ...
    carrito.forEach((producto) => {
        ...
        // Mostrar la cantidad disponible junto al botón "Agregar al carrito"
        const cantidadDisponible = productosDisponibles[producto.nombre];
        if (cantidadDisponible > 0) {
            const botonAgregar = document.createElement('button');
            //botonAgregar.textContent = 'Agregar al cdffdarrito';
            //botonAgregar.classList.add('btn', 'btn-primary', 'btn-sm', 'ml-2');
            //botonAgregar.onclick = () => agregarProducto(producto.nombre, producto.precio);
            item.appendChild(cantidadInput);
            item.appendChild(botonEliminar);
            item.appendChild(botonAgregar);
            item.innerHTML += ` (Disponibles: ${cantidadDisponible})`;
        } else {
            item.appendChild(cantidadInput);
            item.appendChild(botonEliminar);
            item.innerHTML += ' (No disponibles)';
        }
